...
def load_tested_creatives(uploaded_file):
    if uploaded_file is not None:
        ...
        return pd.DataFrame(columns=['creative_id', 'channel'])
...
def autotesting_aggregate(new_data, target_roas_d0, target_cpi):
    ...
    required_columns = ['impressions', 'cost', 'installs', 'retention_rate_d1',
                        'retention_rate_d3', 'retention_rate_d7',
                        'custom_cohorted_total_revenue_d0', 'custom_cohorted_total_revenue_d3',
                        'custom_cohorted_total_revenue_d7', 'CPI', 'channel']
    ...
    for col in ['ROAS_d0', 'ROAS_d3', 'ROAS_d7']:
        aggregated_data[col].replace([np.inf, -np.inf], np.nan, inplace=True)
        aggregated_data[col].fillna(0, inplace=True)
    ...
    aggregated_data['ROAS_Mat_D3'].replace([np.inf, -np.inf], np.nan, inplace=True)
    aggregated_data['ROAS_Mat_D3'].fillna(0, inplace=True)
    ...
    aggregated_data['IPM'].replace([np.inf, -np.inf], 0, inplace=True)
...
if new_file and game_code:
    ...
    prev_data = load_tested_creatives(prev_file) if not first_time_run else pd.DataFrame(columns=['creative_id', 'channel'])
